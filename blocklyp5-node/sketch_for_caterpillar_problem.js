...
function draw(){
     ...
     if(show_enabled.localeCompare(checkTrue) > -1){
        ...
        } for(let i=0;i<user_entries.length/2;i++){     serialnumber=serialnumber+1;                                                  } if(show_enabled.localeCompare(checkTrue)>-1 ) { printTable(); } addRow();CopyRow(1);CopyRow(1);  } 
...
{ copy_row_counter= copy_row_counter + 1; print(copy_row_counter); RepeatBuffer=[]; serialnumber = serialnumber + 1; let i =0; for(let columns=0; columns<3;columns++) { RepeatBuffer[i]= table.getString(row_to_be_copied-1,columns),20+80*columns,100+20*row_to_be_copied-1; console.log("CONTNET",RepeatBuffer[i]); i = i + 1; } let newRow=table.addRow(); k = 0; newRow.setString(column_names[0],""+serialnumber); newRow.setString(column_names[1],""+RepeatBuffer[k+1]); newRow.setString(column_names[2],""+RepeatBuffer[k+2]); }  
     ...
     function printTable(slideCounter) {
       for(let c=0;c<1;c++){ text(table.columns,20 + 80*c,80) } 
                                        ...
                                        if (slideshow==false){
                                          for(let rows=0; rows<user_entries.length/2 + 2; rows++){ for(let columns=0; columns<3;columns++) {
...
if (slideshow==true){ 
  slideCounter = slideCounter + 1; clear();for(let c=0;c<1;c++){ text(table.columns,20 + 80*c,80); } for(let rows=0; rows<slideCounter; rows++){ for(let columns=0; columns<3;columns++) { text(table.getString(rows,columns),20+80*columns,100+20*rows); } } if (slideCounter == user_entries.length/2 && show_sum.localeCompare(checkTrue)>-1){ Total(); } if (slideCounter > user_entries.length/2){ console.log("user to be notifide"); } } }
     ...
     function Total(){ 
       ...
       let newRow = table.addRow(); newRow.setString(column_names[0], "Total"); newRow.setString(column_names[1],""+""+2*2); newRow.setString(column_names[2],""+2*2); let rows=number_of_rows; for (let columns = 0; columns < 3; columns++) { text(table.getString(rows, columns), 20 + 80 * columns, 100 + 20 * rows); } }
